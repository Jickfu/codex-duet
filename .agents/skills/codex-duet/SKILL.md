...
Before a new task, run `chatbridge onboard --mode github` from the target project root (or `--mode local` for an explicit LOCAL request). Read each check's next action; resolve local FAIL items without discarding user work, and verify REQUIRED external prerequisites with the selected provider. An exit code of zero means local prerequisites only: `taskReady` remains false because no GitHub access or Browser login was tested. For an existing task, inspect its durable status and resume instead of treating onboarding as authorization to initialize it again.
