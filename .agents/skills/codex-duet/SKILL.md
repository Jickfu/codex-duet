...
For `PLAYWRIGHT_CLI`, use only the public Browser Bridge commands `send`, `wait`, `browser attach`, `browser detach`, and `browser doctor`. For `CODEX_BROWSER`, use Codex Desktop's browser capability plus `codex-browser-prepare`, `codex-browser-complete`, and `codex-browser-receive`; record `OUTCOME_UNKNOWN` whenever send confirmation is uncertain and never replay it automatically. Use `chatbridge duet` for lifecycle guards. Never inspect ChatGPT DOM/selectors, invoke Playwright internals, start another Codex agent, or require Codex CLI or a Codex SDK.
