...
2. Run `chatbridge duet init --task <taskId> --request-file <request.md> --task-spec-file <task-spec.json> --output <planning-envelope.txt>`. Chatbridge validates identity, raw-request SHA, exact literal preservation, GitHub context, and integrity; it never invents intent. `C2C_PAYLOAD_TOO_LARGE` is a pre-Browser stop condition. Never truncate or split the projection. The raw request and TaskSpec remain local and gitignored.
...
The local TaskSpec is authoritative and the bound conversation is only a semantic cache. `CHATGPT_CONVERSATION_UNAVAILABLE` fails closed; M3.2c Phase 1 has no automatic or explicit rebind. Historical tasks without TaskSpec resume through their existing durable envelopes and are never synthesized or migrated silently.
