...
Compact-task identity is independently pinned in `.chatbridge/runs/<taskId>/task-context.json`. Strict `TaskContextV1` records only task ID, TaskSpec SHA-256, and the first Planner control SHA-256. It is atomically create-or-verify and immutable. Before normal M2 review preparation, orchestration verifies the marker, TaskSpec integrity and pinned fingerprint, and the byte-exact persisted Planner control fingerprint. A marker with a missing or divergent TaskSpec fails closed before `getReviewTarget`; Crash-B adoption performs the same read-only semantic preflight and never repushes. Only absence of both marker and TaskSpec selects historical legacy behavior.
