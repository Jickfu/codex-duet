...
Diff path coverage includes allowed HEAD paths and current paths, preserving staged deletions and rename deletion halves. Every file patch is rendered from its captured current bytes and immutable HEAD blob/mode using two fixed temporary filenames. Workspace paths never serve as Git pathspec security boundaries. Only patch metadata is relabeled to the validated logical path; hunks and binary payloads are unchanged. Sensitive descendants cannot enter these isolated inputs, even during file/directory replacement. Untracked executable changes affect the snapshot fingerprint without staging.

The canonical index authority also includes the ignore-policy fingerprint: effective info/exclude, configured/default global excludes content and hashed location, and all allowed workspace .gitignore rule files (including ignored rule files). External rule contents and absolute paths are never exposed. Rule-file absence is explicit; changing the effective policy changes snapshotId even before any affected source exists. Rule sources obey size/stability checks and the complete policy is recollected during the second capture pass.
