...
After first-send discovery passes the reservation preflight, the task-aware path re-pins that exact canonical URL before login, prepare, or send. Playwright CLI recovery then inspects only that prepared conversation; it never accepts a newer user message from the current page or another ChatGPT candidate. If the exact page is unavailable after a possible send side effect, recovery returns no marker and the caller reports `SEND_OUTCOME_UNKNOWN`. Unscoped M1 recovery keeps its Frozen broad candidate scan for compatibility.
...
Unbound bootstrap is protected by a bounded project-wide filesystem lock covering selection, reservation preflight, send confirmation, and atomic sidecar persistence. Active conflicts are rejected before `sendMessage`. Historical conversations require explicit bootstrap. Confirmed Browser side effects whose task checkpoint cannot be persisted return `SEND_CHECKPOINT_PERSIST_FAILED` and never authorize resend.
