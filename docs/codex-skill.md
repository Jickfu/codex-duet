...
Status: **M3.0 through M3.3 Frozen; real dual-provider Desktop E2E acceptance passed**
...
The raw request remains plaintext, local, and gitignored at `.chatbridge/runs/<taskId>/request.md`. TaskSpecV1 is immutable at `.chatbridge/runs/<taskId>/task-spec.json`; identical torn-init replay is verified and reused, while divergent content fails. Init detects persisted TaskSpec, TaskContext, or first Planner control before consulting the optional current `--task-spec-file`: existing Compact evidence can only recover Compact semantics and never silently initialize legacy. A persisted TaskSpec can recover without the CLI candidate; marker or Planner evidence without TaskSpec fails closed. Immutable `task-context.json` pins the TaskSpec and first Planner control fingerprints so loss or corruption cannot silently become legacy behavior. Exact Planner and Reviewer controls plus byte count and SHA-256 evidence are persisted per iteration. The local TaskSpec is authoritative; the bound conversation is only a semantic cache. New compact envelopes are limited to 8192 UTF-8 bytes over the complete serialized C2C message. `C2C_PAYLOAD_TOO_LARGE` stops before Browser send; content is never truncated, split, or weakened. Only historical tasks with no Compact artifacts retain legacy envelopes and resume semantics.
...
- At durable `EXECUTING`, run `duet reconcile-execution --task <taskId>` and follow its evidence-backed action. Never replay the PLAN blindly or discard existing work.
