...
| M3.2b     | `EXECUTING` Crash Reconciliation         | **FROZEN / REAL DESKTOP CRASH E2E PASS** |
| M3.2c     | Compact Browser Control and Resume UX    | **FROZEN / REAL DESKTOP E2E PASS**       |
| M3.3      | Task Interaction Policy and Discussion   | **FROZEN / REAL DESKTOP E2E PASS**       |
...
M3.3 froze at implementation baseline `7658708c1424b212733fdeb625f4bdb3fca332d8` after the full automated gate passed: 348 of 348 tests, lint, typecheck, build, targeted Prettier, and `git diff --check`.

Real Desktop acceptance passed both required provider dimensions. The `PLAYWRIGHT_CLI` regression task `m3-3-playwright-dogfood-final-v3-20260902` completed the normal lifecycle without Discussion on branch `agent/task-m3-3-playwright-dogfood-final-v3-20260902`; its immutable `REVIEW_REF` is `60d381c63853c585e705ba2854af1060df836fb6`, and the range added only `docs/acceptance/m3-3-playwright-cli.md`. The `CODEX_BROWSER` task `m3-3-codex-browser-discussion-final-v3-20260902` completed one strict Discussion round before planning and then reached `DONE` through the normal Planner, execution, and Reviewer lifecycle. Its immutable evidence branch is `agent/task-m3-3-codex-browser-discussion-final-v3-20260902`, its `REVIEW_REF` is `14674980937adad8a7a378bae9ac1f040c02a4b3`, and the range added only `docs/acceptance/m3-3-codex-browser-discussion.md`.

The CODEX_BROWSER acceptance bound `PREPARED`, `ATTEMPTED`, `CONFIRMED`, and `RESPONDED` checkpoints to one concrete conversation and never replayed a send. Its strict DiscussionResponseV1 returned `CONVERGED` before the final Planner message. The Reviewer accepted the immutable range with no correction. The PLAYWRIGHT_CLI route retained Frozen `BrowserAutomationSession` ownership and existing task-aware send/wait behavior. Neither route used provider fallback, and both acceptance branches remain unmerged immutable evidence. Public documentation intentionally omits private conversation and message identifiers.
