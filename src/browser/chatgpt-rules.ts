  ...
  return `async page => {
    ...
    try{
      ...
      if(c.operation.kind==='commit'){
        ...
        const prepareSend=async()=>{const deadline=Date.now()+10000;let observed=false;while(Date.now()<deadline){const candidates=await step(()=>target.$$(c.selectors.send));if(candidates.length>0)observed=true;const visible=[];for(const candidate of candidates)if(await step(()=>candidate.isVisible()))visible.push(candidate);for(let index=0;index<visible.length;index++){const candidate=visible[index];try{const remainingCandidates=visible.length-index;const trialTimeout=Math.max(1,Math.min(2000,Math.floor((deadline-Date.now())/remainingCandidates)));await step(()=>candidate.click({trial:true,timeout:trialTimeout}));return {kind:'button',handle:candidate}}catch(error){guard()}}await delay(50)}if(observed)fail('CHATGPT_SEND_NOT_READY');return {kind:'keyboard',handle:composer}};
