...
function orchestrator(): DuetOrchestrator {
  ...
  return new DuetOrchestrator(
    new GitHubCodeProvider(new GitRunner(cwd), 'origin', stateRoot),
    new DuetRunStore(stateRoot),
  );
}

export async function duetInit(task: string, requestFile: string, output: string): Promise<void> {
  console.log(JSON.stringify(await orchestrator().init(task, requestFile, output), null, 2));
}
